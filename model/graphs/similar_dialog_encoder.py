...
class Model(object):
  ...
  def _get_pretrained_variables(self):
    self.bert_pretrained_word_embeddings = tf.get_variable(
      name="bert_pretrained_word_embeddings",
      initializer=[v for v in tf.global_variables() if v.name == "bert/embeddings/word_embeddings:0"][0],
      trainable=True,
    )
  ...
  def _similar_dialog_lstm(self, similar_dialog_input_ph, similar_dialog_len_ph):
    ...
    # similar_dialog_fw, similar_dialog_bw = sequence_feature(similar_dialog_out, similar_dialog_len)
    # similar_concat = tf.concat([similar_dialog_fw, similar_dialog_bw], axis=-1)
    # simiar_concat = tf.reshape(similar_concat, shape=[batch_size, top_n, self.hparams.rnn_hidden_dim * 2])
  ...
  def build_graph(self):
    ...
    for each_dialog_out, each_dialog_len in zip(unstacked_similar_dialog_lstm_out, unstacked_similar_dialog_len):
      ...
      esim_att = ESIMAttention(self.hparams, self.hparams.dropout_keep_prob,
                               text_a=response_bert_outputs, text_a_len=response_len,
